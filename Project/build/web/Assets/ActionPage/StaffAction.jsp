...
//checking if request cotains multipart data
            ...
            //f_item=(FileItem)itr.next();
                        ...
                        if (field_name.equals("file_photo"))//write the name of image here it is img_product
                        {
                            ...
                            String file_path = proj_path.getParentFile().getParentFile().getPath() + "\\Web\\Assets\\Files\\StaffPhoto\\";//path for storing image
                            Random r = new Random();//to generate random number
                            int r_num = r.nextInt(1111) + 999;//values between this are generated

                            photo = "Staff_" + r_num + ext;//generating random number for images so this name and format of imagewil be stored inside the server
                        ...
                        if (field_name.equals("file_proof"))//write the name of image here it is img_product
                      {
                          ...
                          //setting path to store image
                          File proj_path = new File(config.getServletContext().getRealPath("/"));
                          String file_path = proj_path.getParentFile().getParentFile().getPath() + "\\Web\\Assets\\Files\\Staffproof\\";//path for storing image
                          Random r = new Random();//to generate random number
                         int r_num = r.nextInt(1111) + 999;//values between this are generated
           ...
           //to upload proof of doctor
               ...
               // {//Strinh str1="insert into tbl_user ()";//inserting file
                 String InsQry = "insert into tbl_staff(staff_name,staff_contact,staff_email,staff_address,staff_pincode,staff_photo,staff_idproof,designation_id,place_id,staff_username,staff_password,temple_id)values('" + value[0] + "','" + value[1] + "','" + value[2] + "','" + value[3] + "','" + value[7] + "','" +photo + "','"+proof+"','" + value[4] + "','" + value[6] + "','"+ value[8] +"','"+value[9]+"','"+session.getAttribute("Tid")+"')";
   ...
   //System.out.println(InsQry);
        ...
        <script type="text/javascript">
            ...
            setTimeout(function() {
                window.location.href = '../../Temple/StaffRegistration.jsp'
